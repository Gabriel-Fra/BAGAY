...
- [ ] **Java Command API parity.** Port `chain.compute_hash` and `commands.submit` to Spring
      ...
      **In progress, unverified.** `java-command-api/` has a dependency-free Java port
      (`Chain`, `Jcs`, `Json`, `Catalog`, `Commands`) plus `tools/export_hash_vectors.py` and a
      `ParityCheck` harness that diffs Java-computed hashes against a real 1,333-event seeded
      log. Written in a sandbox with no `javac` and no network, so **it has not actually been
      compiled or run** — do that next (`java-command-api/build.sh`) before checking this off.
      No Spring Boot/Maven wiring yet (no network there either to resolve dependencies); it's
      plain `javac`-compilable Java meant to drop into Spring Boot service classes unchanged.
      See `java-command-api/README.md`.
